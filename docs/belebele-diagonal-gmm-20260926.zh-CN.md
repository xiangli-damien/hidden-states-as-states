...
- 用户已明确确认三语言合并，每个模型拟合一张逐层地图；保留语言元数据，方便比较同一地图的语言分布。
...
源代码 `f4f5f47` 已推送GitHub并在Lambda fast-forward，服务器针对性检查 **19/19通过**。启动时间 **2026-09-26 05:53:49 UTC**，PID **513889**；实际命令与配置哈希保存在结果根`launch.json`。初始状态`auditing_collection`，随后准备均值缓存、GPU数值预检、逐层拟合。不要重复启动，当前进度以`queue_status.json`为准。配置SHA256：`fbd43ea94c4871e575bc34f4f9838002f20d1e0efd5b2eb4ae5ed9220948d3c4`。
