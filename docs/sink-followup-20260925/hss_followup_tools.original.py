...
def manifold_controls(centroids_nonsink, k: int = PARAMS["n_ctrl_man"], seed: int = PARAMS["seed"] + 2):
    ...
    return V / np.linalg.norm(V, axis=1, keepdims=True), chosen
...
def chart_vectors(bases, v):
    ...
    ok = norms > 1e-6 * np.linalg.norm(v)
...
def _deltas(screen, metric, qset, conds):
    ...
    wide = wide[conds + [NONE_COND]].dropna()   # paired, equal n across all compared conditions
...
def step2_decision(screen: pd.DataFrame, c0_generation_beats_random: bool, params: dict = PARAMS) -> dict:
    ...
    for C in CANDIDATES:
        ...
        per[C] = dict(
            ...
            mean_d_loop=float(d_loop[C].mean()),
    ...
    if miscalibrated:
        ...
        for C in new_pass:
            ...
            tie_ci[C] = [lo_d, hi_d]
            if lo_d <= 0:                       # not clearly worse than the best -> tied
                tied.append(C)
...
def decision_table_markdown(decision: dict) -> str:
    ...
    for C, r in decision["per_candidate"].items():
        lines.append(
            ...
            f"| {r['mean_d_loop']:.3f} | {r['mean_d_col_ans']:.3f} ({r['ctrl_min_d_col_ans']:.3f}) "
