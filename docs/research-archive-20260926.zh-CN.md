...
归档代码 `ea6f244` 已通过 GitHub 部署，六项测试在本地和服务器均通过。真实 R2 测试包18,882,560 bytes，三 multipart、三个文件实际回读 SHA256、完成对象复用全部通过。首次等待器遇到 NFS ESTALE 后保留失败，加入原子状态文件读取重试；当前 PID520470，实际PID以launch.json为准。
