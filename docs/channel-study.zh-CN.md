...
在划分前按合并空白后的完整 prompt 去重，只保留固定分片/行顺序的第一次出现，
选择不查看正误标签。MATH 无重复；MMLU 原始 14,042 条中有 105 条重复，
主分析使用 13,937 个唯一 prompt。原始采集与全量评估结果均保留。
重合 prompt 的跨 subject 重复不会跨发现/验证集；gold 冲突组另外记录在身份审计中。
...
在确定性前向中，同一个 prompt 与完全相同的生成前缀应产生相同的早期状态，未来回答不能
反过来改写它（忽略数值误差）。因此，即使这个前缀后来能随机续写出正确和错误两种答案，
首 token 状态也不能区分那两个尚未分叉的未来。早期信号应解释为成功倾向或输入相关状态，
不能解释为已经知道某个实际未来答案的真伪。
...
1. 固定候选层/通道/方向；对一小组独立问题重复生成，取得**同一道题**的正确与错误轨迹，
   排除问题难度与内容变化。先检查标签及回答解析。共享前缀的状态应一致，可作为无未来
   信息泄漏的检查；不能根据未来的正误标签虚构共享前缀已经不同。
2. 对明确固定的 teacher-forced 前缀分解 attention 输出、MLP 输出；投影到候选残差方向，
   记录每个 block 的增量，进一步用 MLP down-projection 映射内部 neuron 对该方向的贡献。
   轨迹分叉后的正误比较包含 token 内容变化，应单独控制，不能当作只改变“正确性”。
