...
两种差值都完成了拟合。浏览入口：`http://127.0.0.1:8776/report/index.html`。

| 表示 | 主 ICL 成分数 | 换初始化的测试 ARI | 80%训练题目重拟合 ARI |
|---|---|---|---|
| 跨层平均更新，28 层 | 16–48，大部分32 | 层中位数0.447 | 层×种子中位数0.429 |
| 末层跨 token，pre | 80，搜索上限 | 0.590 | 0.509 / 0.597 |
| 末层跨 token，post | 80，搜索上限 | 0.519 | 0.479 / 0.566 |

跨层平均更新逐层 K：`32,32,32,32,32,32,32,32,32,48,48,32,32,32,32,32,32,16,16,16,16,32,32,32,32,32,32,32`。这是粗网格 ICL 选择，不能声称精确最优 K。

**有可重复的分组和正误占比差异，但没有证明自然离散的“失败操作”。**

- 28个平均更新层均通过无条件及长度×熵条件置换的本轮BH校正；加入题型、难度的粗分层后27/28通过。静态mean基线五层也全部通过，因此不能仅凭显著性宣称更新量优于原始状态。没有在本轮训练正确性分类器，也没有证明对长度／熵预测器有增益。
- pre/post跨token的正误JS分别为0.01750/0.01545 bits；长度×熵控制后的超出置换均值部分仅0.00559/0.00410 bits。再加题型、难度后的q为0.00455/0.01143，效应仍小；分箱不能排除连续混杂。
- token簇与前后token类型转换的NMI为0.496/0.510，与相对位置的NMI为0.0153/0.0168。这说明词法转换是明显相关因素；NMI不能解释成“50%由词法解释”，本轮没有完全控制具体token身份。
- token簇后验平均最大值约0.999，但欧氏silhouette只有0.0058/0.0118，且ARI仅中等。尖锐后验不等于簇彼此孤立或重复拟合稳定。
- 第7/14/28层的单高斯对照仍被对角GMM分为16/8/16个成分，测试似然也改善。因此多成分本身可能部分来自相关协方差，不足以证明多峰或离散操作。
- 全部相邻token差值的有符号均值基线JS更大，但它严格退化成端点差除以长度，不能据此声称保留了中间推理过程。

所有候选、选中模型、归属和后验保存于Dami，并同步本地`results/change-clusters-20260921`。调整CPU线程只改变执行资源：跨层使用6进程×2线程；跨token后半程复用已完成候选，以2进程×8线程继续。GPU采集未改动。
