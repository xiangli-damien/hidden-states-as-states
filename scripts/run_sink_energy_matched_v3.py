...
class MatchedTransform:
    ...
    def __call__(self, h):
        ...
        active = target > 0
        repaired = torch.zeros_like(active)
        if self.direction is None:
            ...
            # A BF16 ray has discontinuities: one coordinate can jump across the
            # desired norm. Repair only failing tokens by deterministic outward
            # rounding, minimizing distance cost per energy to the same ideal
            # direction. This does not inspect logits or relax the energy gate.
            allowed = self.cfg['internal_relative_norm_target']*target+self.cfg['internal_absolute_norm_target']
            bad = ((after.float()-x).norm(dim=-1)-target).abs() > allowed
            repaired = bad.clone()
            if bool(bad.any()):
                before = x[bad]
                corrected = candidate(lo)[bad]
                target_bad = target[bad]
                ideal = before-target_bad[:,None]*self.direction
                toward = torch.full_like(corrected, float('inf')) * (-self.direction.sign()).to(h.dtype)
                for _ in range(self.cfg['rounding_repair_steps']):
                    delta = corrected.float()-before
                    energy = delta.square().sum(-1)
                    gap = (target_bad.square()-energy).clamp_min(0)
                    need = (energy.sqrt()-target_bad).abs() > allowed[bad]
                    if not bool(need.any()):break
                    adjacent = torch.nextafter(corrected, toward)
                    step = adjacent.float()-corrected.float()
                    increment = (delta+step).square()-delta.square()
                    cost = (adjacent.float()-ideal).square()-(corrected.float()-ideal).square()
                    eligible = (increment>0) & (increment<=gap[:,None]) & need[:,None]
                    priority = torch.where(eligible,cost/increment.clamp_min(1e-30),torch.full_like(cost,float('inf')))
                    order = priority.argsort(dim=-1,stable=True)
                    sorted_inc = increment.gather(1,order)
                    sorted_ok = eligible.gather(1,order)
                    sums = torch.where(sorted_ok,sorted_inc,torch.zeros_like(sorted_inc)).cumsum(1)
                    take = sorted_ok & (sums<=gap[:,None])
                    mask = torch.zeros_like(take).scatter_(1,order,take)
                    corrected = torch.where(mask,adjacent,corrected)
                after[bad] = corrected
        ...
        intended = -self.axis if self.direction is None else -self.direction
        control_cosine = (actual_delta @ intended)/actual.clamp_min(1e-30)/intended.norm()
        self.arrays = dict(target_norm=target.cpu().numpy(), actual_norm=actual.cpu().numpy(),
            ...
            rounding_repaired=repaired.cpu().numpy(), actual_cosine_intended=control_cosine.cpu().numpy())
        self.metrics = dict(n_tokens=len(h), active=int(active.sum()), same_active_mask=same_mask,
            ...
            rounding_repaired_tokens=int(repaired.sum()),
            actual_intended_cosine_energy_weighted=float((control_cosine*actual.square()).sum()/actual.square().sum().clamp_min(1e-30)),
