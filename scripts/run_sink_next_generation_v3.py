...
from sink_online_rounding_v3 import MatchedTransform,selftest,preflight
...
def prepare():
    root=BASE/'exp4-v3';root.mkdir(parents=True,exist_ok=True)
    ...
    code=['run_sink_next_generation_v3.py','sink_online_rounding_v3.py','run_sink_next_generation_v2.py','sink_online_rounding_v2.py','run_sink_next_generation.py','report_sink_next_generation.py','run_sink_next.py','run_sink_energy_matched_v3.py',
    ...
    files += [REPO/'scripts'/n for n in code]+[REPO/'docs/sink-next-protocol-20260925.zh-CN.md',REPO/'docs/sink-next-generation-numerical-amendment-v2-20260925.md',REPO/'docs/sink-next-generation-numerical-amendment-v3-20260925.md']
    original=BASE/'exp4-v2'
    ...
    plan=dict(config=cfg,files={str(p):sha(p) for p in files},frozen_utc=now(),git=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
        numerical_amendment='v3: preserve v2 passing operations; extend failed numerical rounding iterations to 128/512/2048; same outer gates',imported_records=[str(p.relative_to(original)) for p in imported],previous=str(previous),sink_local=prep['sink_local'],expected_records=606,seed=SEED,bootstrap_replicates=100000,
...
def run():
    ...
    write_json(root/'numeric_preflight_SUCCESS.json',preflight(cfg))
