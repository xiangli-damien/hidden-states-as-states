...
def prepare():
    root=BASE/'exp4';root.mkdir(parents=True,exist_ok=True)
    ...
    code=['run_sink_next_generation.py','report_sink_next_generation.py','run_sink_next.py','run_sink_energy_matched_v3.py',
    ...
    plan=dict(config=cfg,files={str(p):sha(p) for p in files},frozen_utc=now(),git=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
        previous=str(previous),sink_local=prep['sink_local'],expected_records=606,seed=SEED,bootstrap_replicates=100000,
