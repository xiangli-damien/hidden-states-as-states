...
def render(root):
    ...
    m=pd.read_csv(root/'layer_metrics.csv').rename(columns={'k_percent_0':'k_percent_0.0'}).sort_values('position')
    ...
    for ax in axs.flat:style(ax);ax.set_xticks(ticks,ticklabels,rotation=75,ha='right');ax.set_xlabel('Layer / final norm side')
    ...
    for ax,title in zip(axs.flat,['Between-cluster / total centered variance','Euclidean silhouette: same 1024 questions','Frozen-map occupied components','K under common score-difference budgets']):
        ax.set(title=title);ax.legend(fontsize=8);ax.set_xticks(ticks,ticklabels,rotation=75,ha='right');style(ax)
    ...
    for row in m.to_dict('records'):
        ...
        ax=axs[0,2];ax.plot(np.arange(1,129),e[:128]/e.sum());ax.set(yscale='log',title='Variance spectrum: first 128 of 3584 directions',xlabel='Principal direction',ylabel='Fraction of total variance')
        ...
        ax.xaxis.set_major_locator(LogLocator(base=10,subs=(1,2,5),numticks=7))
        ...
        ax.set(title='Same 12000 question pairs',xlabel='Cosine similarity',ylabel='Density');ax.legend(fontsize=8)
        for ax in axs.flat:style(ax)
        fig.suptitle(f'{unit}: PR={row["effective_dimension"]:.2f}; first 2 PCs explain {100*e[:2].sum()/e.sum():.1f}% — projection is not a clustering test',fontsize=15)
    ...
    columns=['unit','effective_dimension','d90','diagonal_effective_dimension','mean_energy_fraction','offdiagonal_covariance_fraction','radius_q99_gaussian_ratio','log_length_variance_fraction','k_percent_0.0','k_percent_0.02','k_delta_0.05','mfa_k']
    ...
    names={'unit':'位置','effective_dimension':'全局PR','d90':'90%方差方向数','diagonal_effective_dimension':'只保留对角项PR','mean_energy_fraction':'公共均值能量占比',
...
<h2>3. 逐层查看</h2><label for="unit">选择位置：</label><select id="unit">'''+options+'''</select><img id="detail" src="L00.png" alt="逐层分布详情"><p>PCA散点图仅显示前两个方向，各层坐标轴独立拟合；MFA颜色对应本层局部簇号，不表示跨层同一状态。图中的色块不构成离散状态的证据。</p>
...
<script>document.getElementById('unit').onchange=e=>{document.getElementById('detail').src=e.target.value+'.png'};</script></main></html>'''
    ...
    save_json(root/'report_audit.json',dict(valid=True,units=checks,metrics_rows=30,partition_rows=120,
        renderer_sha256=file_digest(__file__),
