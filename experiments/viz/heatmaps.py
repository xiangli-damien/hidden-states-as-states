...
from .style import COLORS, COLOR_BIRTH, COLOR_DEATH, CMAP_DIVERGING, CMAP_PROBABILITY, save_figure


def plot_heatmap_diverging(
    data: np.ndarray, layers: np.ndarray, cluster_order: List[int],
    birth: Dict[int, int], death: Dict[int, int], cbar_label: str,
    vmax: Optional[float] = None, figsize: Tuple[float, float] = (7, 5),
    out_path: Optional[Path] = None,
) -> plt.Figure:
    sub = data[:, cluster_order]
    finite = sub[np.isfinite(sub)]
    if vmax is None:
        vmax = max(float(np.nanmax(np.abs(finite))) if finite.size else 0.1, 1e-3)
    ...
    nC, L = len(cluster_order), len(layers)
    step = max(1, nC // 12)
    ...
    for xi, gid in enumerate(cluster_order):
        ...
        if b in layer_to_row and layer_to_row[b] > 0:
            ax.plot(xi, layer_to_row[b], marker="v", markersize=4, color=COLOR_BIRTH,
                    markeredgecolor="white", markeredgewidth=0.4, alpha=0.9)
            has_b = True
        if d in layer_to_row and layer_to_row[d] < L - 1:
            ax.plot(xi, layer_to_row[d], marker="^", markersize=4, color=COLOR_DEATH,
                    markeredgecolor="white", markeredgewidth=0.4, alpha=0.9)
            has_d = True
    if has_b or has_d:
        elems = []
        if has_b:
            elems.append(Line2D([0], [0], marker="v", color="w", markerfacecolor=COLOR_BIRTH,
                                markersize=6, label="Birth", linestyle="None"))
        if has_d:
            elems.append(Line2D([0], [0], marker="^", color="w", markerfacecolor=COLOR_DEATH,
                                markersize=6, label="Death", linestyle="None"))
...
def plot_heatmap_probability(
    data: np.ndarray, layers: np.ndarray, cluster_order: List[int],
    birth: Dict[int, int], death: Dict[int, int], baseline: float,
    cbar_label: str, figsize: Tuple[float, float] = (7, 5),
    out_path: Optional[Path] = None,
) -> plt.Figure:
    sub = data[:, cluster_order]
...
def plot_dual_axis_information(
    df_info: pd.DataFrame, figsize: Tuple[float, float] = (9, 4.5),
    out_path: Optional[Path] = None,
) -> plt.Figure:
    ...
    ax1.set_ylabel(r"$H(C_\ell)$ (bits)", color=c1, fontsize=10)
    l1, = ax1.plot(layer, H_C, marker="o", markersize=5, color=c1, lw=1.5, label=r"$H(C_\ell)$")
    ax1.tick_params(axis="y", labelcolor=c1)
    ax2 = ax1.twinx()
    ax2.set_ylabel(r"$I(C_\ell; Y)$ (bits)", color=c2, fontsize=10)
    l2, = ax2.plot(layer, I_CY, marker="s", markersize=5, color=c2, lw=1.5, label=r"$I(C_\ell; Y)$")
    ax2.tick_params(axis="y", labelcolor=c2)
    iv = I_CY[np.isfinite(I_CY)]
    if iv.size > 0 and float(np.max(iv)) > HY * 0.5:
        ax2.axhline(HY, color=c2, linestyle="--", alpha=0.5, lw=1.0)
    ax1.grid(True, alpha=0.25, axis="both", linestyle=":")
    ax1.legend([l1, l2], [l1.get_label(), l2.get_label()], loc="upper center", ncol=2, frameon=True, fontsize=8)
    plt.tight_layout()
    if out_path:
        save_figure(fig, out_path)
    return fig
...
def plot_information_bottleneck(
    df_info: pd.DataFrame, figsize: Tuple[float, float] = (5.5, 5),
    out_path: Optional[Path] = None,
) -> plt.Figure:
    fig, ax = plt.subplots(figsize=figsize)
    ...
    sc = ax.scatter(x, yv, c=l, cmap="viridis", norm=norm, s=50, edgecolors="black", linewidths=0.4, alpha=0.95)
    ...
    fig.colorbar(sc, ax=ax, fraction=0.04, pad=0.02).set_label("Layer", fontsize=9)
    plt.tight_layout()
    if out_path:
        save_figure(fig, out_path)
    return fig
...
def plot_effective_rank(
    df: pd.DataFrame, figsize: Tuple[float, float] = (8, 4),
    out_path: Optional[Path] = None,
) -> plt.Figure:
    fig, ax = plt.subplots(figsize=figsize)
    ...
    for i, frac in enumerate(fracs):
        ...
        ax.plot(agg["layer"], agg["mean"], marker="o", markersize=3, color=color, lw=1.2,
                label=f"{frac:.0%} (n={n_sub})")
        ax.fill_between(agg["layer"], agg["mean"] - agg["std"], agg["mean"] + agg["std"],
...
def plot_rel_icl_surface(
    rel_mat: np.ndarray, layers: np.ndarray, k_values: np.ndarray,
    k_sel_by_layer: Optional[Dict[int, int]] = None, rel_pct: float = 0.02,
    ...
    vmax = max(float(np.quantile(finite, 0.95)) if finite.size else 0.2, rel_pct * 2.0, 0.05)
    im = ax.imshow(rel_mat, aspect="auto", interpolation="nearest", cmap="magma_r",
    ...
    if k_sel_by_layer:
        ...
        for i, layer in enumerate(layers):
            ...
            j = k_to_j.get(int(ks)) if ks is not None else None
            if j is not None:
                xs.append(j)
                ys.append(i)
        if xs:
