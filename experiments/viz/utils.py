...
def add_curved_edge(
    ...
    path_data = [
        ...
        (MplPath.CURVE4, (cx1, y1)),
        (MplPath.CURVE4, (cx2, y2)),
        (MplPath.CURVE4, (x2, y2)),
    ]
    codes, verts = zip(*path_data)
    path = MplPath(verts, codes)
    patch = PathPatch(
        path, facecolor="none", edgecolor=color, lw=lw, alpha=alpha,
...
def compute_positions(
    ...
    layers_list = [int(l) for l in layers]
    for li, layer in enumerate(layers_list):
        ...
        for i, c in enumerate(sorted(clusters)):
            x = li * x_spacing
            y = y0 + i * y_spacing
            positions[(layer, c)] = (float(x), float(y))
    return positions
